...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
      ...
      /*  for(Object key : keys) {
            ((countries.get(key)).startsWith("R"))
            {
                seekingCountries.add(countries.get(key));
            }
        }*/
    ...
    public List<String> iteratingOverEntries(){
        ...
        for(Map.Entry<String,String> in : countries.entrySet()){
            if(in.getKey().contains("Q") || in.getKey().contains("q"))
                seekingCountries.add(in.getKey().toLowerCase());
